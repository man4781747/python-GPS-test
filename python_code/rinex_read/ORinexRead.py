...
#            print('test')
        ...
        if S_GNSS_num > 12:
#            print('test')
#            S_GNSS_info = Lst_each_title_read[7][I_GNSS_jump:] + Lst_readlines[I_title_num+1].split()[0]
            S_GNSS_info = Lst_readlines[I_title_num_chose][32:].strip() + Lst_readlines[I_title_num_chose+1][32:].strip()
#            print(S_GNSS_info)
            I_title_num_chose += 1
        else:
    #        S_GNSS_info = Lst_each_title_read[7][I_GNSS_jump:]
...
#                        print(Lst_observ[I_L1_loc][14] == '1' )
...
#                    print('L1 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    ...
                    pass
    #                print('L2 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    Ay_gps_P1_sing[0,I_gps_num] = float(Lst_observ[I_P1_loc][:14])
                except:
                    pass
    #                print('P1 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    Ay_gps_P2_sing[0,I_gps_num] = float(Lst_observ[I_P2_loc][:14])
#                    print(Lst_observ[I_P2_loc])
                except:
                    pass
    #                print('P2 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    Ay_gps_C1_sing[0,I_gps_num] = float(Lst_observ[I_C1_loc][:14])
#                    print(Lst_observ[I_C1_loc])
                except:
                    pass
    #                print('C1 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    Ay_gps_C2_sing[0,I_gps_num] = float(Lst_observ[I_C2_loc][:14])
                except:
                    pass
    #                print('C2 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    Ay_gps_S1_sing[0,I_gps_num] = float(Lst_observ[I_S1_loc][:14])
                except:
                    pass
    #                print('S1 data lose? In line {0}'.format(I_title_num_chose))
                try:
                    Ay_gps_S2_sing[0,I_gps_num] = float(Lst_observ[I_S2_loc][:14])
                except:
                    pass
    #                print('S2 data lose? In line {0}'.format(I_title_num_chose))
            if I_numofOBStype > 5:
                I_title_num_chose += 2
            elif I_numofOBStype <= 5:
                I_title_num_chose += 1
    except:
#        print(Lst_readlines[I_title_num_chose])
        I_title_num_chose += 1
...
#    print(len(np.where(Ay_gps_L1_sing!=0)))
