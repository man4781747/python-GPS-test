...
while (true)
    ...
    if strfind(line,'APPROX POSITION XYZ')                                  % Receiver aprox position
        XYZ_station=[real(str2double(line(2:14))),real(str2double(line(16:28))),real(str2double(line(30:42)))];
...
%         data = [gpsWeek, currentEpoch,currentFlag,prn,measurementsValue];   % store data
        data = [currentEpoch,prn,measurementsValue]; %data = [tow, svid, pr, carrierphase, dopplerfrequency, snr]
        obs{nObs} = data;
        nObs= nObs+1;
    end
